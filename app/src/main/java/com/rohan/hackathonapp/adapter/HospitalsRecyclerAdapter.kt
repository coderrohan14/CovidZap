...
class HospitalsRecyclerAdapter(val context: Context, val hospitalList: ArrayList<Hospital>):
    RecyclerView.Adapter<HospitalsRecyclerAdapter.HospitalsViewHolder>(){
    class HospitalsViewHolder(view: View):RecyclerView.ViewHolder(view){
    ...
    override fun onBindViewHolder(holder: HospitalsRecyclerAdapter.HospitalsViewHolder, position: Int) {
        ...
        holder.distance.text = currHospital.distance.toString()
    }
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): HospitalsViewHolder {
        val view = LayoutInflater.from(parent.context).inflate(R.layout.recycler_hospitals_single_row,parent,false)
        return HospitalsRecyclerAdapter.HospitalsViewHolder(view)
    }
}